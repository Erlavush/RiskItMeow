...
- The shop dynamically scans `assets/props/low_poly_household`, exposing `112` imported FBX props across `14` categories, on top of the legacy starter items.
...
- Imported FBX props now load into the browser and placement system, but many still rely on generic scale/collision heuristics and may need per-item tuning.
